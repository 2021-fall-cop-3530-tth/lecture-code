...
const Point startPosition(60, 40, 220);
static Ship theShip(startPosition);
...
void explode(int v = 0) {
  glClearColor(0.8, 0.4, 0.2, 1.0);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  wireframe ? landscape.drawWireFrame() : landscape.draw();
  cockpit.draw(true); // make it red
  glFlush();
  glutSwapBuffers();
}
...
void timer(int v) {
  theShip.fly();
  Point currentPosition = theShip.getPosition();
  if (currentPosition.y < landscape.getElevation(currentPosition.x, currentPosition.z))
  {
    std::cout << "Ship crashed!\n";
	explode();
  }
  else
  {
...
void reshape(int w, int h) {
  //glViewport(0, 0, w, h);
  glViewport(0, 0, 2 * w, 2 * h); // retina display
